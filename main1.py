...
# NGROK TUNNEL MODE
# ========================================
# Global flag: set to True when /start is hit via ngrok
_ngrok_start_event = threading.Event()

def start_ngrok_server():
    """
    Start Flask + ngrok tunnel and wait for /start command.
    When /start is received via the public ngrok URL, the OTDR
    test sequence is triggered.
    """
    if not FLASK_AVAILABLE:
        print("❌ Flask is required for ngrok mode. Install with: pip3 install Flask Flask-CORS")
        return
    if not PYNGROK_AVAILABLE:
        print("❌ pyngrok is required for ngrok mode. Install with: pip3 install pyngrok")
        return

    # --- Build a separate Flask app for the ngrok listener ---
    ngrok_app = Flask("ngrok_listener")
    CORS(ngrok_app)

    @ngrok_app.route('/', methods=['GET'])
    def ngrok_home():
        """Home page — shows status."""
        if test_in_progress:
            status_text = "🔄 OTDR test is currently RUNNING..."
        elif _ngrok_start_event.is_set():
            status_text = "✅ Start command already received — test is running."
        else:
            status_text = "⏳ Waiting for start command."
        ...
        <style>
          ...
          .card {{ background: #1e293b; padding: 3rem; border-radius: 1rem; text-align: center;
                   box-shadow: 0 25px 50px rgba(0,0,0,0.4); max-width: 500px; }}
          h1 {{ color: #38bdf8; margin-bottom: 0.5rem; }}
          .status {{ font-size: 1.2rem; margin: 1.5rem 0; padding: 1rem; background: #334155;
        ...
        <body>
          <div class="card">
            ...
            <a class="btn" href="/start">🚀 START OTDR TEST</a>
          </div>
        </body>
        </html>
        """
    ...
    def ngrok_start():
        """Receive start command — triggers OTDR test sequence."""
        global test_in_progress
        if test_in_progress:
            return """
            ...
            <p>Please wait for the current test to finish.</p></div></body></html>
            """, 409

        _ngrok_start_event.set()
        print("\n" + "=" * 60)
        ...
        <p style='color:#94a3b8;margin-top:1rem;'>You can close this tab.</p>
        </div></body></html>
        """, 200
    ...
    print("\n" + "=" * 60)
    print("🌐 NGROK TUNNEL MODE")
    print("=" * 60)

    try:
        pyngrok_conf.get_default().auth_token = NGROK_AUTHTOKEN
        public_url = pyngrok_ngrok.connect(NGROK_PORT, "http").public_url
    except Exception as e:
        print(f"❌ Failed to open ngrok tunnel: {e}")
        return

    print(f"\n🔗 PUBLIC URL (paste this in your laptop browser):")
    print(f"\n   ➜  {public_url}\n")
    print(f"   To start the OTDR test, open:")
    print(f"   ➜  {public_url}/start\n")
    print("=" * 60)
    print("⏳ Waiting for start command...\n")

    # Run Flask in a background thread
    server_thread = threading.Thread(
        target=lambda: ngrok_app.run(host='0.0.0.0', port=NGROK_PORT, debug=False, use_reloader=False),
        daemon=True
    )
    server_thread.start()
    ...
    try:
        while True:
            if _ngrok_start_event.wait(timeout=1):
                break
    except KeyboardInterrupt:
        print("\n🛑 Interrupted by user. Shutting down ngrok...")
        pyngrok_ngrok.kill()
        return

    # --- /start was received — run the OTDR test ---
    ...
    # Keep server alive briefly so user can check /health, then shut down
    print("\n⏳ Server will remain available for 30 seconds before shutdown...")
    time.sleep(30)
    pyngrok_ngrok.kill()
...
def main():
    ...
    Usage:
        ...
        python main.py --ngrok          # Ngrok tunnel mode (remote start via browser)
    ...
    if len(sys.argv) == 1 or (len(sys.argv) == 2 and sys.argv[1] == "--server"):
    ...
    elif len(sys.argv) == 2 and sys.argv[1] == "--ngrok":
        # --ngrok flag: start ngrok tunnel and wait for /start
        start_ngrok_server()
        ...
        print("\n  2. NGROK TUNNEL MODE (Remote Start via Browser)")
        print("     python main.py --ngrok")
        print("     • Opens ngrok tunnel with public URL")
        print("     • Paste the URL in any browser to see control page")
        print("     • Click START or visit /start to trigger OTDR test")
        print("     • No port-forwarding or local network needed")
