...
NGROK_PORT = 5001               # Port for ngrok tunnel listener
DASHBOARD_URL = os.environ.get("DASHBOARD_URL", "http://10.61.71.205:8080/api/status")  # Dashboard on laptop

# Ngrok Configuration
NGROK_AUTHTOKEN = os.environ.get("NGROK_AUTHTOKEN", "3DnjndJRYEI3JJdmTuqTvS0oqCr_nmJqwVFUz5wcqR7S9Xei")
NGROK_DOMAIN = os.environ.get("NGROK_DOMAIN", "relieve-transport-spoiling.ngrok-free.dev")  # Your fixed ngrok domain
...
# 4) fallback to thejotech host on port 8081
...
if _backend_env:
    ...
    if _backend_env.startswith("http"):
        ...
        if ":" in _backend_env:
            LOCAL_SERVER_URL = f"http://{_backend_env}/api/otdr/upload"
        else:
            LOCAL_SERVER_URL = f"http://{_backend_env}:3000/api/otdr/upload"
else:
    LOCAL_SERVER_URL = os.environ.get("BACKEND_DEFAULT", "http://thejotech.com:3000/api/otdr/upload")

print(f"🔗 Using backend upload URL: {LOCAL_SERVER_URL}")

# API client token for /api/otdr/upload (set BACKEND_API_TOKEN to override)
LOCAL_SERVER_TOKEN = os.environ.get("BACKEND_API_TOKEN", "hardware_client_token_12345")
    ...
    def convert_and_upload(self, sor_file_path):
        ...
        try:
            ...
            if self.upload_callback:
                try:
                    success = self.upload_callback(json_file_path, sor_file_path)
                except TypeError:
...
dashboard_communicator = DashboardCommunicator(DASHBOARD_URL)

def _ensure_dashboard_communicator():
    global dashboard_communicator
    if dashboard_communicator is None:
        dashboard_communicator = DashboardCommunicator(DASHBOARD_URL)
    return dashboard_communicator

def _build_status_payload(include_history=True):
    dash = dashboard_communicator
    status = dash.get_current_status() if dash else "IDLE"
    history = dash.get_status_history() if dash and include_history else []
    return {
        "status": status,
        "history": history,
...
def _mark_run_start():
    global test_in_progress, last_run_ok, last_error, last_started_at, last_completed_at
    test_in_progress = True
    last_run_ok = None
    last_error = None
    last_started_at = datetime.now(timezone.utc).isoformat()
    last_completed_at = None
    dash = _ensure_dashboard_communicator()
    dash.clear_status()
    dash.send_status("STARTED", "OTDR test sequence started", progress=0)

def _mark_run_end(success, error_message=None):
    global test_in_progress, last_run_ok, last_error, last_completed_at
    test_in_progress = False
    ...
    dash = _ensure_dashboard_communicator()
    if success:
        dash.send_status("COMPLETE", "OTDR test sequence completed", progress=100)
    else:
        dash.send_status("ERROR", error_message or "OTDR test failed", progress=100)

if FLASK_AVAILABLE:
    app = Flask(__name__)
    # Allow any origin/host/port for dashboard access (no credentials with wildcard)
    CORS(
        app,
        resources={r"/*": {"origins": "*"}},
        supports_credentials=False,
        allow_headers=["Content-Type", "Authorization", "X-Requested-With", "X-API-Key", "ngrok-skip-browser-warning"],
    ...
    def add_cors_headers(response):
        ...
        response.headers["Access-Control-Allow-Headers"] = "Content-Type, Authorization, X-Requested-With, X-API-Key, ngrok-skip-browser-warning"
        return response
    ...
    def stop_test():
        ...
        _mark_run_end(False, "Test stopped by user")
...
# ========================================
# NGROK TUNNEL MODE (PERSISTENT SERVER)
# ========================================

def _run_otdr_in_background():
    """Run OTDR test sequence in a background thread and reset state when done."""
    global test_in_progress
    try:
        print("\n🚀 Executing OTDR test sequence...\n")
        success = run_otdr_test_sequence()
        if success:
            print("\n" + "=" * 60)
            print("✅ OTDR TEST COMPLETED SUCCESSFULLY")
            print("=" * 60)
        else:
            print("\n" + "=" * 60)
            print("❌ OTDR TEST FAILED")
            print("=" * 60)
    except Exception as e:
        print(f"❌ OTDR test error: {e}")
    finally:
        test_in_progress = False
        print("\n⏳ Server is still running. Waiting for next /start command...\n")

def start_ngrok_server():
    """
    Start Flask + ngrok tunnel with persistent server.
    The server stays alive forever. Each GET /start triggers
    the OTDR test in a background thread. After the test
    completes, the server continues waiting for the next command.
    """
    if not FLASK_AVAILABLE:
        print("❌ Flask is required. Install with: pip3 install Flask Flask-CORS")
        return
    if not PYNGROK_AVAILABLE:
        print("❌ pyngrok is required. Install with: pip3 install pyngrok")
        return

    # --- Build Flask app ---
    ngrok_app = Flask("otdr_remote")
    # Allow any origin/host/port for ngrok UI access (no credentials with wildcard)
    CORS(
        ngrok_app,
        resources={r"/*": {"origins": "*"}},
        supports_credentials=False,
    ...
    @ngrok_app.after_request
    def add_ngrok_cors_headers(response):
        response.headers["Access-Control-Allow-Origin"] = "*"
        response.headers["Access-Control-Allow-Methods"] = "GET, POST, PUT, PATCH, DELETE, OPTIONS"
        response.headers["Access-Control-Allow-Headers"] = "Content-Type, Authorization, X-Requested-With, X-API-Key, ngrok-skip-browser-warning"
        return response

    @ngrok_app.route('/', methods=['GET'])
    def ngrok_home():
        """Home page — shows status and START button."""
        if test_in_progress:
            status_text = "🔄 OTDR test is currently RUNNING..."
            btn_class = "btn btn-disabled"
            btn_text = "⏳ TEST IN PROGRESS..."
            btn_href = "#"
        else:
            status_text = "✅ Ready — waiting for start command."
            btn_class = "btn"
            btn_text = "🚀 START OTDR TEST"
            btn_href = "/start"

        html = f"""
        <!DOCTYPE html>
        <html>
        <head><title>OTDR Remote Control</title>
        <meta http-equiv="refresh" content="5">
        <style>
          body {{ font-family: 'Segoe UI', sans-serif; background: #0f172a; color: #e2e8f0;
                 display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; }}
          .card {{ background: #1e293b; padding: 3rem; border-radius: 1rem; text-align: center;
                   box-shadow: 0 25px 50px rgba(0,0,0,0.4); max-width: 520px; width: 90%; }}
          h1 {{ color: #38bdf8; margin-bottom: 0.5rem; }}
          .status {{ font-size: 1.1rem; margin: 1.5rem 0; padding: 1rem; background: #334155;
                     border-radius: 0.5rem; }}
          a.btn {{ display: inline-block; margin-top: 1rem; padding: 1rem 2.5rem;
                   background: linear-gradient(135deg, #22c55e, #16a34a); color: white;
                   text-decoration: none; border-radius: 0.75rem; font-size: 1.1rem;
                   font-weight: 600; transition: transform 0.2s; }}
          a.btn:hover {{ transform: scale(1.05); }}
          a.btn-disabled {{ background: #475569; pointer-events: none; cursor: not-allowed; }}
          .info {{ color: #64748b; font-size: 0.8rem; margin-top: 1.5rem; }}
        </style></head>
        <body>
          <div class="card">
            <h1>🔬 OTDR Remote Control</h1>
            <p>Raspberry Pi OTDR Automation</p>
            <div class="status">{status_text}</div>
            <a class="{btn_class}" href="{btn_href}">{btn_text}</a>
            <p class="info">Page auto-refreshes every 5 seconds</p>
          </div>
        </body>
        </html>
        """
    ...
    @ngrok_app.route('/start', methods=['GET'])
    def ngrok_start():
        """Receive start command — triggers OTDR test in background thread."""
        global test_in_progress
        if test_in_progress:
            return """
            <!DOCTYPE html><html><head><title>OTDR</title>
            <meta http-equiv="refresh" content="5;url=/">
            <style>body{font-family:sans-serif;background:#0f172a;color:#fbbf24;
            display:flex;justify-content:center;align-items:center;height:100vh;}
            .card{background:#1e293b;padding:3rem;border-radius:1rem;text-align:center;}</style>
            </head><body><div class='card'><h1>⚠️ Test Already Running</h1>
            <p>Please wait for the current test to finish.</p>
            <p style='color:#64748b;margin-top:1rem;'>Redirecting back in 5 seconds...</p>
            </div></body></html>
            """, 409

        test_in_progress = True
        print("\n" + "=" * 60)
        print("🚀 START COMMAND RECEIVED VIA NGROK TUNNEL!")
        print("=" * 60 + "\n")

        # Run OTDR test in background thread so server stays responsive
        test_thread = threading.Thread(target=_run_otdr_in_background, daemon=True)
        test_thread.start()

        return """
        <!DOCTYPE html><html><head><title>OTDR Started</title>
        <meta http-equiv="refresh" content="5;url=/">
        <style>body{font-family:sans-serif;background:#0f172a;color:#22c55e;
        display:flex;justify-content:center;align-items:center;height:100vh;}
        .card{background:#1e293b;padding:3rem;border-radius:1rem;text-align:center;}
        </style></head><body><div class='card'>
        <h1>✅ OTDR Test Started!</h1>
        <p>The Raspberry Pi is now executing the OTDR test sequence.</p>
        <p style='color:#94a3b8;margin-top:1rem;'>Redirecting to status page in 5 seconds...</p>
        </div></body></html>
        """, 200

    @ngrok_app.route('/health', methods=['GET'])
    def ngrok_health():
        return jsonify({
            "status": "online",
            "timestamp": datetime.now().isoformat(),
    ...
    @ngrok_app.route('/status', methods=['GET'])
    def ngrok_status():
        payload = _build_status_payload(include_history=False)
        payload["timestamp"] = datetime.now().isoformat()
        return jsonify(payload), 200

    # --- Configure & open ngrok tunnel ---
    print("\n" + "=" * 60)
    print("🌐 OTDR REMOTE SERVER (NGROK TUNNEL)")
    print("=" * 60)

    try:
        pyngrok_conf.get_default().auth_token = NGROK_AUTHTOKEN
        # Use fixed domain so the URL never changes
        public_url = pyngrok_ngrok.connect(
            NGROK_PORT, "http",
            hostname=NGROK_DOMAIN
        ).public_url
    except Exception as e:
        print(f"❌ Failed to open ngrok tunnel: {e}")
        print("   Make sure your ngrok authtoken is valid and the domain is claimed.")
        return

    print(f"\n🔗 YOUR PERMANENT URL (never changes):")
    print(f"\n   ➜  https://{NGROK_DOMAIN}")
    print(f"\n   To trigger OTDR test, open in any browser:")
    print(f"   ➜  https://{NGROK_DOMAIN}/start")
    print(f"\n📌 Server will keep running. Send /start as many times as you need.")
    print(f"   Press Ctrl+C to shut down.")
    print("=" * 60 + "\n")

    # Run Flask (this blocks — keeps server alive forever)
    try:
        ngrok_app.run(host='0.0.0.0', port=NGROK_PORT, debug=False, use_reloader=False)
...
def run_otdr_test_sequence():
    ...
    try:
        ...
        dash.send_status("INITIALIZING", "Setting up GPIO pins", progress=5)
        ...
        dash.send_status("PROCESSING", "Checking for new OTDR files", progress=65)
        ...
        if not latest_folder:
            ...
            if not unconverted:
                dash.send_status("NO_NEW_FILES", "No new .sor files found", progress=75)
                print("⚠️ [WARNING] No new .sor files found")
            else:
                dash.send_status("CONVERTING", f"Found {len(unconverted)} new file(s)", progress=75)
                ...
                if had_processing_error:
                    return fail_run("One or more files failed to process")
                
        dash.send_status("UNBINDING", "Disconnecting OTDR before power off", progress=85)
        ...
        _mark_run_end(False, "Test sequence interrupted by user")
        gpio.cleanup_gpio()
        return False
        ...
        import traceback
        traceback.print_exc()
        _mark_run_end(False, f"Unexpected error: {str(e)}")
        gpio.cleanup_gpio()
        return False


# API Authentication Settings (legacy JWT flow, used only if no BACKEND_API_TOKEN is set)
...
def upload_json_to_backend(json_file_path, sor_file_path=None):
    """
    Upload JSON file and SOR file to backend database.
    Args:
        json_file_path (str): Path to JSON file
        sor_file_path (str): Path to SOR file (optional)
    Returns:
        bool: True if successful, False otherwise
    """
    try:
        ...
        if sor_file_path and os.path.exists(sor_file_path):
            with open(sor_file_path, 'rb') as f:
                payload["sor_file_name"] = os.path.basename(sor_file_path)
                payload["sor_file_base64"] = base64.b64encode(f.read()).decode('ascii')

        print(f"📤 Uploading to: {url}")
        try:
            response = requests.post(url, json=payload, headers=headers, timeout=30)
        except requests.exceptions.SSLError as ssl_err:
            if url.startswith("https://") and "WRONG_VERSION_NUMBER" in str(ssl_err):
                http_url = "http://" + url[len("https://"):]
                print(f"⚠️  HTTPS failed (wrong TLS version). Retrying over HTTP: {http_url}")
                response = requests.post(http_url, json=payload, headers=headers, timeout=30)
            else:
                raise
        
        # If unauthorized, try to clear token and retry once (JWT flow only)
        if response.status_code in (401, 403) and not BACKEND_TOKEN:
...
def manual_parse_mode():
    ...
    if parser.parse_sor_file():
        ...
        if json_file:
            ...
            print("=" * 40)
            upload_json_to_backend(json_file, sor_file)
...
def main():
    ...
    Usage:
        python main.py                  # Default: Ngrok server mode (persistent, reusable)
        python main.py <file.sor>       # Manual mode (parse single file)
        python main.py --auto           # Automated mode (run once)
        python main.py --local          # Local-only server (no ngrok, port 5000)
        python main.py --help           # Show help
    """
    
    if len(sys.argv) == 1:
        # DEFAULT: start ngrok tunnel server (persistent, reusable)
        start_ngrok_server()

    elif len(sys.argv) == 2 and sys.argv[1] == "--local":
        # --local flag: run local dashboard server only (no ngrok)
        print("\n🌐 Starting in LOCAL SERVER mode - no ngrok tunnel...\n")
        ...
        print("=" * 60)
        print("\nModes of Operation:")
        print("  1. DEFAULT MODE (Ngrok Remote Server) ⭐")
        print("     python main.py")
        print(f"     • Opens ngrok tunnel at: https://{NGROK_DOMAIN}")
        print("     • Paste the URL in any browser to see control page")
        print("     • Click START to trigger OTDR test")
        print("     • Server stays running — send /start as many times as you want")
        print("     • Press Ctrl+C to shut down")
        print("\n  2. LOCAL SERVER MODE (LAN only)")
        print("     python main.py --local")
        print("     • Starts HTTP server on port 5000 (no ngrok)")
        print("     • Only accessible on local network")
        print("     • POST /trigger to start test")
        print("\n  3. AUTOMATED MODE (Single Test)")
        ...
        print("\n  4. MANUAL PARSE MODE (Development/Testing)")
        print("     python main.py <file.sor>")
        print("     • Parse single OTDR file")
        print("     • Generate JSON + upload to backend")
        print("=" * 60)
